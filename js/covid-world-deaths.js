...
//function covidVis(data) {
 // D3 code chunk
    ...
    var yScale = d3.scaleLinear()
          //.domain([ d3.min(data, d => d.Deaths7),
          .domain([ 0, d3.max(data, d => d.Deaths7) ])
    ...
    var line = d3.line()
          ...
          .y(function(d) { return yScale(d.Deaths7); })
    
    
    // Don't need to define this variable for r2d3
    //var svg = d3.select("#covid-container")
    //    .append("svg")
    //    .attr("width", width)
    //    .attr("height", height);
    ...
    svg.selectAll(".line")
        ...
        .attr("d", function(d) {
            return d3.line()
                .defined(function(d) { return d.Deaths7 >= 0; })
                .x(function(d) { return dateScale(parseTime(d.date)); })
                .y(function(d) { return yScale(d.Deaths7); })
                (d.values);
        })
    ...
    svg.append("circle")
        ...
        .attr("cy", d => yScale(d.Deaths7))
        .attr("r", 5);
    svg.append("text")
        .data(data_usamax)
        .attr("class", "usa-text")
        .attr("x", d => dateScale(parseTime(d.date))+15)
        .attr("y", d => yScale(d.Deaths7)+5)
        .text(d => d.country);
